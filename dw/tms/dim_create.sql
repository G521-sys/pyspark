...
create external table dim_truck_driver_full(
  ...
  `is_enabled` tinyint COMMENT '状态 0：禁用 1：正常'
    ...
    partitioned by (`dt` string comment '统计日期')
    stored as orc
    location '/bigdata_warehouse/tms/dim/dim_truck_driver_full'
    tblproperties('orc.compress'='snappy');
...
create external table dim_truck_full(
  ...
  `is_enabled` tinyint COMMENT '状态 0：禁用 1：正常'
    ...
    partitioned by (`dt` string comment '统计日期')
    stored as orc
    location '/bigdata_warehouse/tms/dim/dim_truck_full'
    tblproperties('orc.compress'='snappy');
...
create external table dim_user_zip(
  ...
  `end_date` string COMMENT '结束日期'
    ...
    partitioned by (`dt` string comment '统计日期')
    stored as orc
    location '/bigdata_warehouse/tms/dim/dim_user_zip'
    tblproperties('orc.compress'='snappy');
...
create external table dim_user_address_zip(
  ...
  `end_date` string COMMENT '结束日期'
    ...
    partitioned by (`dt` string comment '统计日期')
    stored as orc
    location '/bigdata_warehouse/tms/dim/dim_user_address_zip'
    tblproperties('orc.compress'='snappy');
select version()